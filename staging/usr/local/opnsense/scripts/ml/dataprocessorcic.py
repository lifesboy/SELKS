...
def create_processor_pipe(data_files: [], batch_size: int, num_gpus: float, num_cpus: float):
    ...
    convert_options = csv.ConvertOptions(include_columns=schema.keys(), column_types=schema)
...
# ex: /usr/bin/python3 /usr/local/opnsense/scripts/ml/dataprocessor.py --data-source=nsm/*.csv --batch-size=500 --num-gpus=0.4 --num-cpus=0.1 --data-destination=nsm
# ex: /usr/bin/python3 /usr/local/opnsense/scripts/ml/dataprocessorcic.py --data-source=cic2018/*.csv --batch-size=500 --batch-size-source=1 --num-gpus=0.4 --num-cpus=0.1 --data-destination=cic2018 --tag=cic2018
